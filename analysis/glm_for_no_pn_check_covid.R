...
############### 
## model with Charlson Y/N, no hbp_pregnancy history + covid time ###############
## traditional glm()
model_full <- glm(postnatal_8wk_code_present ~ Age+BMI+Region+Ethnicity+IMD+charlsonGrp2+covid, data = df_input, family = binomial(link = "logit"))
...
write_csv(fit_results, here::here("output","mod_full_Charlson_covid.csv"))

############### 
## model with Charlson Y/N AND hbp_pregnancy history 
###############
#  mod full 
model_full_hbp <- glm(postnatal_8wk_code_present ~ Age+BMI+Region+Ethnicity+IMD+charlsonGrp2+hbp_pregnancy+covid, data = df_input, family = binomial(link = "logit"))
...
write_csv(fit_results_hbp, here::here("output","mod_full_Charlson_AND_hbp_covid.csv"))
...
write_csv(fit_covid_results, here::here("output","mod_full_Charlson_covid_interaction.csv"))