...
#  short model  
## traditional glm()
model_charlsonGrp2 <- glm(postnatal_8wk_code_present ~ charlsonGrp2, data = df_input, family = binomial(link = "logit"))

# Extract coefficient estimates and exponentiate them
fit_results <- tidy(model_charlsonGrp2, exponentiate = TRUE)

# Extract confidence intervals and exponentiate them
conf_intervals <- confint(model_charlsonGrp2)
...
write_csv(fit_results, here::here("output","mod_charlsonGr2_only.csv"))
