...
# write csv for rates
write_csv(as.data.frame(df_plot), here::here("output", "ITS_plot_data_imd_edit.csv"))

# df for each category
df1=filter(df_plot, imd=="1")
df2=filter(df_plot, imd=="2")
df3=filter(df_plot, imd=="3")
df4=filter(df_plot, imd=="4")
df5=filter(df_plot, imd=="5")
...
# creates combined df with estimates and CIs
...
write_csv(as.data.frame(df_plot_overall), here::here("output", "ITS_plot_imd_IRR_overall_edit.csv"))
...
DF_counter= rbind(df1_counter_final,df2_counter_final,df3_counter_final,df4_counter_final,df5_counter_final)
DF_counter$imd=factor(DF_counter$imd,levels=c("1","2","3","4","5"))

#DF_counter=DF_counter%>%filter(date>=as.Date("2020-04-01"))

### plot 
plot_ITS_imd_edit<-ggplot(DF_plot_f, aes(x=date, y=fit*1000/population, group=covid))+ 
  theme_bw()+ 
...
#plot_ITS
ggsave(
  plot= plot_ITS_imd_edit,
  filename="plot_ITS_imd_edit.jpeg", path=here::here("output"), dpi = 300
  )

write.csv(DF,here::here("output","plot_ITS_check_imd_edit.csv"))
