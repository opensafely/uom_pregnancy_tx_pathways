...
#  short model  
## traditional glm()
model_age <- glm(postnatal_8wk_code_present ~ Age, data = df_input, family = binomial(link = "logit"))
model_agecat <- glm(postnatal_8wk_code_present ~ age_cat, data = df_input, family = binomial(link = "logit"))

# Extract coefficient estimates and exponentiate them
fit_results <- tidy(model_age, exponentiate = TRUE)
fit_results_cat <- tidy(model_agecat, exponentiate = TRUE)

# Extract confidence intervals and exponentiate them
conf_intervals <- confint(model_age)
exp_conf_intervals <- exp(conf_intervals)
conf_intervals_cat <- confint(model_agecat)
...
write_csv(fit_results, here::here("output","mod_age.csv"))
write_csv(fit_results_cat, here::here("output","mod_age_cat.csv"))
