...
variables_names_comor <- df_input %>% 
                  dplyr::select(Age, BMI, Region, Ethnicity, IMD, 
                                ...
                                "mod_severe_liver", "mod_severe_renal", "mi",
                                "peptic_ulcer" , "peripheral_vascular")
explanatory_vars=colnames(variables_names_comor)
dependent="postnatal_8wk_code_present"


df_input %>%
  finalfit.glm(dependent, explanatory_vars, add_dependent_label = F,
               dependent_label_prefix= "", metrics = TRUE) -> modComor
modComor.df <- as.data.frame(modComor)
modComor.dfr  <- modComor.df [,-c(3:5)]
write_csv(modComor.dfr, here::here("output","mod_full_Comor17_finalfit.csv"))
