...
#  short model  
## traditional glm()
model_covid_only <- glm(postnatal_8wk_code_present ~ covid, data = df_input, family = binomial(link = "logit"))

# Extract coefficient estimates and exponentiate them
fit_results <- tidy(model_covid_only, exponentiate = TRUE)

# Extract confidence intervals and exponentiate them
conf_intervals <- confint(model_covid_only)
...
write_csv(fit_results, here::here("output","mod_covid_only.csv"))
