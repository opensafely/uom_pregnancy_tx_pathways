...
############### 
## model with age, bmi, eth, imd, region
###############
#  short model  
## traditional glm()
model_agebmiethimdreg <- glm(postnatal_8wk_code_present ~ age + bmi + Ethnicity + imd + region, data = df_input, family = binomial(link = "logit"))


# Extract coefficient estimates and exponentiate them
fit_results <- tidy(model_agebmiethimdreg, exponentiate = TRUE)

# Extract confidence intervals and exponentiate them
conf_intervals <- confint(model_agebmiethimdreg)
...
write_csv(fit_results, here::here("output","mod_agebmiethimdreg.csv"))
