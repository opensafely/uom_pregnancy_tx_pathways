...
#write_csv(as.data.frame(df_gaps), here::here("output", "pn_check_combined_plot_12wk.csv"))
write_csv(as.data.frame(df_gaps), here::here("output", "pn_check_combined_plot_12wk_updated.csv"))
...
plot_pn_rate <- ggplot(df_gaps, aes(x=date))+
  ...
  labs(
    title = "Rate of PN checks within 12 weeks by month",
...
# ggsave(
#    plot= plot_pn_rate,
#    filename="monthly_pn_rate_measures_12wk.jpeg", path=here::here("output"),
# )

ggsave(
   plot= plot_pn_rate,
   filename="monthly_pn_rate_measures_12wk_updated.jpeg", path=here::here("output"),
)
